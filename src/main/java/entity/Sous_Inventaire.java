...
public class Sous_Inventaire {
	...
	public int CurrentSousInventoryNumber[] = new int[999]; // créer un tableau de 999 element pour mettre la quantité
															// d'item a la même position que l'item dans l'autre tableau
	...
	// fonction qui permet de savoir la quantité d'un item en particulier dans le
	// sous inventaire
	...
	// fonction qui permet d'ajouter un item en plusieurs exemplaires (Quantity)
	// dans l'inventaire
	...
	// fonction qui permet de savoir la quantité d'objet (y compris en plusieurs
	// exemplaire) dans l'inventaire
	...
	public void showInventory() {
		...
		for (int i = 0; i < 999; i++) {
			if (this.CurrentSousInventory[i] != null) {
				println("Item ID: " + i + " | " + this.CurrentSousInventory[i].getNom() + " | "
						+ CurrentSousInventoryNumber[i]);
			} else {
				println("Item ID: " + i);
			}
		}
	...
	// fonction dev qui permet d'affichier le sous inventaire dans la console de
	// manière simple
	public String showInventorySimple() {
		for (int i = 0; i < 999; i++) {
			if (this.CurrentSousInventory[i] != null) {
				return "Item ID: " + i + " | " + this.CurrentSousInventory[i].getNom() + " | "
						+ CurrentSousInventoryNumber[i];
			}
		}
		return null;
	...
	// fonction dev qui permet de recuperer le sous inventaire sous forme
	// d'arraylist d'item
