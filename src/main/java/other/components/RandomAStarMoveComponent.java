...
/**
 * Classe qui permet de déplace une entité aléatoirement
 * 
 * @author Pitohui.G7
 *
 */
...
public class RandomAStarMoveComponent extends Component {
	...
	public RandomAStarMoveComponent(Duration minInterval, Duration maxInterval) {
		moveIntervalSupplier = () -> Duration
				.seconds(FXGLMath.random(minInterval.toSeconds(), maxInterval.toSeconds()));
	...
	public void onUpdate(double tpf) {
		if (moveTimer.elapsed(moveInterval)) {
			if (astar.isAtDestination()) {
				astar.getGrid().getRandomCell(c -> c.getState().isWalkable()).ifPresent(astar::moveToCell);
			}
